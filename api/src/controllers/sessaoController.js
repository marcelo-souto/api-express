...
const sessaoController = {
    ...
    create:async (req,res)=>{
        ...
        try {
            ...
            if(!findAdministrador ) throw new Error('Usuario não tem altorização para essa área');
            
            validate({'id do dia': dia, type: 'numero', isRequired: true });
            validate({'duração em horas': horas.hora, type: 'numero', isRequired: true})
            validate({'duração em minutos': horas.minuto, type: 'numero', isRequired: true})
            ...
            let TerminoHorarioEmMinutos = (sessaoHoraTermino * 60) + sessaoMinutoTermino
            ...
            if(sessaoHoras)return res.status(200).json( `A duração da sessão de ${timeConvert(InicioHorarioEmMinutos)} até ${timeConvert(TerminoHorarioEmMinutos)} está em comflitos com as horas do filme ${sessaoHoras.filme.nome}, cujo a Sessao será de ${timeConvert(sessaoHoras.inicioMinuto)} até ${timeConvert(sessaoHoras.terminoMinuto)}`)
    ...
    update:async(req,res)=>{
        const {administradorId,sessaoId,dia,horas,sala,preco,filme} = req.body
        ...
        try {
            ...
            return res.status(200).json(` A sessão foi atualizada:${SessaoAtualizada}`)
    ...
    delete:async(req,res)=>{
		...
		try {
			...
			if (!sessao) return res.status(404).json({ erro: 'Sessao não encontrada.' });
